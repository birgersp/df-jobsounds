...
void Sound_Mixer::initialize()
{
	if (SDL_Init(SDL_INIT_AUDIO) < 0)
		throw function_exception("Could not initialize SDL audio");

	if (Mix_OpenAudio(22050, MIX_DEFAULT_FORMAT, 2, 4096) < 0)
		throw function_exception("Could not initialize SDL audio mixer");
...
void Sound_Mixer::play(const Sound& sound)
{
	...
	if (channel == -1)
		throw function_exception("Error playing sound " + std::to_string(sound.id) + ": " + Mix_GetError());
}
