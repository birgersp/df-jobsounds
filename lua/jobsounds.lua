...
local function initSocket() socket = tcp:connect('127.0.0.1', 56730) end
...
local function checkDwarves()
    for k, unit in ipairs(df.global.world.units.active) do
        if (dfhack.units.isDwarf(unit)) then handleDwarf(unit) end
    end
end
...
function loop()
    ...
    if (df.global.pause_state == false) then checkDwarves() end
